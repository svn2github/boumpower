...
resourcestring
  pkErrorNotConnected = 'PICkit2 non connecté';
  pkErrorDoLoopNesting = 'Trop de ''Do..Loop'' imbriquées';
  pkErrorNoDoInLoop = '%s sans ''Do''';
  pkErrorArgsNumber = '%s nombre illégal d''arguments %d au lieu de %d';
  pkErrorNullStr = 'Chaîne vide';
  pkErrorIllegalIdentifier = 'Identifier invalide ''%s''';
  pkErrorLabelAlreadyExists = 'L''étiquette ''%s'' existe déjà';
  ...
  pkWarningStrTruncated = 'Chaîne tronquée %s en ''%s''';
  pkWarningDelayedResponse = 'Réponse différée';
...
begin
  ...
  if Assigned(FOnAddFirmwareCommand) then
    FOnAddFirmwareCommand(Self, ID_UPLOAD_DATA,
      FirmwareCommandText(ID_UPLOAD_DATA, '!!!! à programmer !!!'));
