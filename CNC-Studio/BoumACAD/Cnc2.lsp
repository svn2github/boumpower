...
(defun cnc_proceed_entity(e fulldxf / dxfstr
  ...
  (setq XYZ     '("X" "Y" "Z")
        XYZ1    '("X1" "Y1" "Z1")
        XYZ2    '("X2" "Y2" "Z2")
        dxfstr  (if fulldxf (id_data "DXF" (etos e)) "")
        FlgSEQ  nil                ; T = sequence polyline ou insert
        etype   (dxf 0 e))         ; Type
  (cond
    ((= etype "POINT")
        (poke #CncChnl #CncItem
          (strcat "POINT"
    ...
    ((= etype "SEQEND")
        (poke #CncChnl #CncItem (strcat "SEQEND" dxfstr))
      ) ; SEQEND
  ); cond
